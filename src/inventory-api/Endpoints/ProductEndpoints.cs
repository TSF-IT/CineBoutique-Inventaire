...
RETURNING ""Id"", ""Sku"", ""Name"", ""Ean"";";
    ...
    private static void MapImportProductsEndpoint(IEndpointRouteBuilder app)
    {
        app.MapPost("/api/products/import", () =>
        {
            ...
            {
                ["reason"] = "GLOBAL_IMPORT_DEPRECATED"
            };

            return Results.Problem(
                title: "Import global indisponible",
                detail: "Veuillez utiliser POST /api/shops/{shopId}/products/import pour importer un catalogue.",
        ...
        .WithOpenApi(op =>
        {
            op.Summary = "Import global des produits (déprécié)";
            op.Description = "Cet endpoint est retiré. Ciblez désormais une boutique via POST /api/shops/{shopId}/products/import.";
            return op;
        })

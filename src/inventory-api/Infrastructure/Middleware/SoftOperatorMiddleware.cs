...
{
    public sealed class SoftOperatorMiddleware(RequestDelegate next, ILogger<SoftOperatorMiddleware> logger)
    {
        public const string OperatorContextItemKey = "SoftOperatorMiddleware.OperatorContext";

        private readonly RequestDelegate _next = next ?? throw new ArgumentNullException(nameof(next));
        private readonly ILogger<SoftOperatorMiddleware> _logger = logger ?? throw new ArgumentNullException(nameof(logger));

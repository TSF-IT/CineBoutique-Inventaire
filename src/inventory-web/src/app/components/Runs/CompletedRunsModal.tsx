...
const formatOwnerForTitle = (
  run: CompletedRunSummary,
  detail?: CompletedRunDetail | null
) =>
  toValidOwnerName(detail?.ownerDisplayName ?? run.ownerDisplayName) ??
  "Inconnu";
...
const buildRunTitle = (
  ...
  const completedOn = formatDateOnly(
    detail?.completedAtUtc ?? run.completedAtUtc
  );
...
const buildCsvContent = (title: string, detail: CompletedRunDetail) => {
  const hasSubGroup = detail.items.some(
    (item) =>
      typeof item.subGroup === "string" && item.subGroup.trim().length > 0
  );
  const baseHeader = ["SKU", "EAN", "Libellé"];
  const header = hasSubGroup
    ? [...baseHeader, "Sous-groupe", "Quantité"]
    : [...baseHeader, "Quantité"];
...
const buildGlobalCsvContent = (details: CompletedRunDetail[]) => {
  ...
  const lines = details.flatMap((detail) => {
    ...
    if (!detail.items || detail.items.length === 0) {
      ...
      const cells = hasSubGroup
        ? [...baseCells, "", formatQuantity(0)]
        : [...baseCells, formatQuantity(0)];
      return [cells];
    }
...
export const CompletedRunsModal = ({
  open,
  completedRuns,
  onClose,
  ...
  const [globalExportError, setGlobalExportError] = useState<string | null>(
    null
  );
  ...
  const handleExport = useCallback(() => {
    ...
    const title = buildRunTitle(selectedRun, runDetail, (value) =>
      formatCountTypeCode(value)
    );
  ...
  const handleExportAll = useCallback(async () => {
    ...
    try {
      ...
      for (const run of orderedCompletedRuns) {
        ...
        const detail = cached ?? (await getCompletedRunDetail(run.runId));
      ...
      const isoDate = Number.isNaN(today.getTime())
        ? "export"
  ...
  const detailErrorDescription = useMemo(
    () => describeDetailError(detailError),
    [detailError]
  );
  ...
  const detailZoneCode = selectedRun
    ? toValidLocationCode(selectedRun.locationCode)
    : null;
  const shouldDisplayZoneCode = Boolean(
    detailZoneCode && detailZoneCode !== detailZoneTitle
  );
                ...
                <div className="rounded-2xl border border-emerald-200 bg-white/70 px-4 py-4 text-sm text-slate-700 shadow-sm dark:border-emerald-800/60 dark:bg-emerald-900/20 dark:text-slate-200">
                  <p>
                    ...
                    <span className="font-semibold text-slate-900 dark:text-white">
                      {formatOwnerName(
                        runDetail?.ownerDisplayName ??
                          selectedRun.ownerDisplayName
                      )}
                    </span>
                  </p>
                  <p className="mt-1">
                    ...
                    <span className="font-semibold text-slate-900 dark:text-white">
                      {formatDateTimeLong(
                        runDetail?.completedAtUtc ?? selectedRun.completedAtUtc
                      )}
                    </span>
                  </p>
                  <p className="mt-1">
                          ...
                          {runDetail.items.map((item) => (
                            <tr key={item.productId}>
                              ...
                              <td className="whitespace-nowrap px-4 py-3 text-left text-sm font-semibold text-emerald-700 dark:text-emerald-200 sm:text-right">
                                <span className="table-label text-slate-500 dark:text-slate-300">
                                  Quantité
                                </span>
                                <span className="table-value sm:text-right">
                      ...
                      return (
                        <li key={run.runId}>
                          ...
                          >
                            ...
                            <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                              {describeCountType(run.countType)} • Opérateur :{" "}
                              {formatOwnerName(run.ownerDisplayName)} • Terminé
                              le {formatDateTime(run.completedAtUtc)}
                            </p>
                          </button>
                        </li>
                      );

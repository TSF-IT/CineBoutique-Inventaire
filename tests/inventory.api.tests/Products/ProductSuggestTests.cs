...
public class ProductSuggestTests : IClassFixture<TestApiFactory>
{
  ...
  public async Task Suggest_BySkuPrefix_And_ByNameFragment_Works()
  {
        ...
        if (root.ValueKind == System.Text.Json.JsonValueKind.Array)
        {
            foreach (var el in root.EnumerateArray())
            {
                if (el.TryGetProperty("name", out var nameProp))
                {
                    ...
                    if (name.Contains("C\u00E9", System.StringComparison.Ordinal) || // "Cé"
                        name.Contains("Café", System.StringComparison.Ordinal))
                    {
                        foundCafe = true;
                        break;
                    }
    ...
    Xunit.Assert.True(foundCafe, $"Expected at least one suggestion 'name' to contain 'Café'. Payload: {b}");
  }
}
